...
void Restroom::description(Character *cha, int num) {
    ...
    if(restCh == 1){
        ...
        print();
        std::cout<<"You got strength "<<getPower<<" It's great!"<<std::endl;
        cha->setStrength(cha->getStrength() + getPower);

    }
    else if(restCh == 3) {
        std::cout<<"Oh, this time there is nothing. Sorry! "<<std::endl;
    }

}

void Restroom::print(){
    std::cout<<"********** You are really lucky this time **********"<<std::endl;
    std::cout<<"This time you will get extra strength between 1 and 4."<<std::endl;
    std::cout<<"Let me see. How much strength you are going to get?"<<std::endl;
}